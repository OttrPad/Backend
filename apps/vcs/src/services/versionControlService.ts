...
export const getCommitTimelineService = async (roomId: string) => {
  ...
  const { data, error } = await supabase
    .from('commits')
    .select('commit_id, block_id, file_id, commit_message, created_at, author_id, milestone')
    .eq('room_id', roomId)
  ...
  const timeline = await Promise.all(
    data.map(async (commit: any) => {
      ...
      // Enrich commit data
      commit.author = authorData ? authorData.username || authorData.email : 'Unknown User';
