...
const ROOM_IDLE_TTL_MS = parseInt(
  process.env.COLLAB_ROOM_IDLE_TTL_MS || "300000"
); // 5 min default
const REAPER_INTERVAL_MS = parseInt(
  process.env.COLLAB_REAPER_INTERVAL_MS || "60000"
); // 1 min default
...
async function runIdleReaper() {
  ...
  for (const room of rooms) {
    ...
    if (participantsCount === 0 && idleFor > ROOM_IDLE_TTL_MS) {
      try {
        console.log(`🧹 Reaping idle room ${room.roomId} (idle ${idleFor}ms)`);
