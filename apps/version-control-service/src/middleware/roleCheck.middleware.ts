 ...
 */
export const requireEditorOrOwner = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (error || !userRoles) {
      ...
      return res.status(403).json({ error: "Forbidden", message: "User not authorized" });
    }

    // Check if the access_level is either "editor" or "owner"
    if (userRoles.access_level !== "editor" && userRoles.access_level !== "owner") {
    ...
    console.error("Error in requireEditorOrOwner middleware:", err.message || err);
